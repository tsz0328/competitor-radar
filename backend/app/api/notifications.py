...
def _high_conditions(current_user: User) -> list:
    """当前用户时间窗内的高优事件。"""
    return [
...
async def _count_unread(db: AsyncSession, current_user: User) -> int:
    """当前用户高优事件中的未读数（总高优数 - 已读记录数）。"""
    conditions = _high_conditions(current_user)
    total = (
        await db.execute(
            select(func.count())
...
async def list_notifications(
    ...
    """当前用户的高优事件通知（多端已读态一致）。"""
    return await _query(db, current_user, limit, offset)
...
async def mark_all_read(
    ...
    """全部已读：为当前用户对所有高优事件批量插/更新已读记录（单次事务）。"""
    high_ids = (
        await db.execute(
            select(IntelligenceEvent.id)
            .join(Competitor, Competitor.id == IntelligenceEvent.competitor_id)
            .where(*_high_conditions(current_user))
        )
    ).scalars().all()
    await _mark_read_batch(db, current_user.id, list(high_ids))
    return ReadAckOut(unread=await _count_unread(db, current_user), read_id=None)
